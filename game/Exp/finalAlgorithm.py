...
def LagrangeDualStageIforVop(F):
    ...
    for n in range(cst.max_iteration):
        ...
        f_m = [
            ...
            for m in range(v_number)
        ]
        p_m = [((f_m[i] ** 2 / Theta_m[i]) + sum(
            [((Theta_m[j - 1] ** -1) - (Theta_m[j] ** -1)) * (f_m[j - 1] ** 2) for j in range(1, i+1)])) for i in
               range(v_number)]
        ...
        Phi_m_grad, Omega_m_grad, Rho_m_grad, Pi_grad, Upsilon_j_grad, Lambda_j_grad = caculate_VopGradient(f_m,
                                                                                                            p_j_vop, F,
                                                                                                            f_j_vop,
                                                                                                            Rho_m)
        ...
        if np.allclose(Phi_m_new, Phi_m, atol=cst.Error_value) and np.allclose(Omega_m_new, Omega_m,atol=cst.Error_value) \
                ...
                and np.allclose(Upsilon_j_new, Upsilon_j,atol=cst.Error_value) and np.allclose(Lambda_j_new, Lambda_j,atol=cst.Error_value):
            break
        Phi_m = Phi_m_new
        Omega_m = Omega_m_new
        Rho_m = Rho_m_new
