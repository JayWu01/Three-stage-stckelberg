...
def checkConstrain(f_m, p_m, p_j_vop,F):
    ...
    for j in range(v_number):
        ...
        g5 = v_number * sum([lamda_m[i] * f_m[i] for i in range(v_number)]) - sum([sum(F[j]) - p_j_vop[j] / (2 * a * e[j] * K[j]) for j in range(len(K))])

    for j in range(len(K)):
        g1 = p_j_vop[j]
        g2 = sum(F[j])*a*2*e[j]*K[j]-p_j_vop[j]
    return g1>= 0 and g2>= 0
def caculate_optimal_p_j_vop(p_j_vop,F,Upsilon_j,Lambda_j,Pi):
    p_j_vop_new=0
    while True:
        p_j_vop_grad=[sum(F[j])-p_j_vop[j]/(a*e[j]*K[j]) + Pi/(2*a*e[j]*K[j]) + Upsilon_j[j]-Lambda_j[j] for j in range(len(K))]
        p_j_vop=[p_j_vop[j]+cst.s_k*p_j_vop_grad[j] for j in range(len(K))]

        if np.allclose(p_j_vop_new, p_j_vop, atol=cst.Error_value):
            return p_j_vop
        p_j_vop_new=p_j_vop
    return p_j_vop
...
def LagrangeDualStageIforVop(F):
    ...
    for n in range(cst.max_iteration):
        ...
        # p_j_vop = caculate_optimal_p_j_vop(p_j_vop,F,Upsilon_j,Lambda_j,Pi)
        aa=v_number * sum([lamda_m[i] * f_m[i] for i in range(v_number)])
        b=sum([sum(F[j]) - p_j_vop[j] / (2 * a * e[j] * K[j]) for j in range(len(K))])
        ab=aa-b
        ...
        # Phi_m_new = [0 for i in range(v_number)]
        # Omega_m_new =  [0 for i in range(v_number)]
        # Pi_new = np.maximum(0, Pi - cst.s_k * Pi_grad)
        # Upsilon_j_new =  [0 for i in range(v_number)]
        # Lambda_j_new = [0 for j in range(len(K))]
